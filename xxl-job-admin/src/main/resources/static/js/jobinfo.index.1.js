$(function () {
    ...
    $("#addModal .glueType").change(function () {
        ...
        } else if ('GLUE_GOLANG' == glueType) {
            $("#addModal .form textarea[name='glueSource']").val($("#addModal .form .glueSource_golang").val());
        } else if ('GLUE_DATAX' == glueType) {
            $("#addModal .form textarea[name='glueSource']").val($("#addModal .form .glueSource_datax").val());
    ...
    $("#job_list").on('click', '.update', function () {
        ...
        if (row.jobCheck == 1) {
            $("#updateModal .form input[name='jobCheck']").attr("checked", true);
        }else{
			$("#updateModal .form input[name='jobCheck']").attr("checked", false);
		}
        $("#updateModal .form input[name='jobTime']").val(row.jobTime ? moment(new Date(row.jobTime)).format("YYYY-MM-DD HH:mm:ss") : "");
        $("#updateModal .form input[name='timespan']").val(row.timespan);
        $("#updateModal .form input[name='depend']").val(row.depend);
